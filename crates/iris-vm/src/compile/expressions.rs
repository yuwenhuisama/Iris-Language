...
impl Lowering<'_, '_> {
    ...
    /// Assembles an interpolating regex PATTERN, escaping each spliced value.
    ///
    /// A value spliced into a pattern is matched LITERALLY: `/${x}/` where `x`
    /// holds `a+b` matches the three characters, not `a` repeated. Escaping is
    /// what makes that true, and it happens at run time because the value is
    /// not known before then.
    pub(super) fn interpolated_regex(&mut self, text: &str) -> Result<Register, CompileError> {
        let mut rest = text;
        let mut built = self.load_text("")?;
        while let Some(open) = rest.find("${") {
            let literal = self.load_text(&rest[..open])?;
            ...
            let value = self.expression(expression)?;
            let escaped = self.allocate()?;
            self.instructions.push(Instruction::EscapeRegex {
                destination: escaped,
                value,
            });
            built = self.concat_text(built, escaped)?;
            rest = &after[end + 1..];
        }
        let literal = self.load_text(rest)?;
        self.concat_text(built, literal)

...
## v1.35 Decorator Coverage Obligations

IRIS-V1-CONFORMANCE-C076: Conformance to the approved v1.35 protocol in [chapter 08](08-modules-metaprogramming.md#v135-decorator-invocation-protocol), IRIS-V1-META-C127 through IRIS-V1-META-C139, MUST include the following observable coverage obligations. Each admitted behavior needs positive coverage and each rejection needs negative or diagnostic coverage, with exact clause anchors, applicable backend comparisons, failure phase and side-effect observations under C016/C033/C047/C049. These rows allocate no vector or example IDs and MUST NOT be counted as executable records or passing evidence. Published META V428 through V431 and V361 remain preserved; their narrower committed fixtures do not establish these obligations. Missing coverage MUST remain visible under C057; revision approval under README TRACE-C023 does not waive that gate. Reports MUST distinguish changed semantic expectations from unchanged but incomplete fixtures, without rewriting or retagging them to hide failures.

IRIS-V1-CONFORMANCE-C077: Conformance to the approved v1.36 language revision MUST verify the following normative language obligations:
1. Top-level static `impl Class for Contract` declarations populate both ordinary and qualified Contract slot dispatch under IRIS-V1-TYPES-C100 through IRIS-V1-TYPES-C103.
2. Order-independence of top-level `impl` declarations across Class, Contract, and `impl` definitions under IRIS-V1-TYPES-C101.
3. Empty `impl Class for Contract {}` correctly reuses compatible existing methods and rejects incomplete classes under IRIS-V1-TYPES-C104.
4. Uniqueness, coherence, orphan prevention, and non-overlapping implementation rules under IRIS-V1-TYPES-C105.
5. Rejection of closed generic specialization `impl Box<Integer> for Show` under IRIS-V1-TYPES-C106.
6. Pure declarative origin class and module bodies, and rejection of executable origin statements under IRIS-V1-META-C140 with `PARSE_ORIGIN_BODY_REQUIRES_DECLARATION`.
7. Atomic publication of `open class Target mixin MixedModule` under IRIS-V1-META-C141.
8. Revalidation of static `impl` obligations during open class transactions under IRIS-V1-META-C142.
9. Unique origins and rejection of duplicate origins under IRIS-V1-META-C143 with `QUALIFIED_NAMESPACE_COLLISION`.
10. Module `where Self: Contract` constraint enforcement upon mixin under IRIS-V1-META-C144.
11. Stable diagnostics for all superseded legacy syntax forms under IRIS-V1-MIG-038 through IRIS-V1-MIG-043 (`PARSE_LEGACY_CLASS_FOR`, `PARSE_LEGACY_METHOD_IMPL`, `PARSE_ORIGIN_BODY_REQUIRES_DECLARATION`, `PARSE_ARRAY_PREFIX_REQUIRED`, `PARSE_IS_QUESTION_REQUIRED`).
...
| --- | --- |
| META-C127, META-C139 | All five target Contracts require both phase members; zero-argument construction is separate per application and phase; application arguments reach phase members only; constructor and transitive planner nondeterminism reject statically before runtime/publication; all kind Symbols/reason Symbols are readable; retained empties keep tags; outside-phase construction rejects; provable wrong kind is static and runtime-only wrong kind is candidate validation, with full rollback and no consumed revision/commit. |
| META-C128, META-C129 | Factory and fluent forms produce equivalent inert persistent descriptions; old values do not change; multiple additions compose in order; getter plus setter combines in one Property result; repeated wrappers preserve first-written-outermost order; A then B phase order gives A(B(original)), reversed source reverses nesting; Logged/Cached/Retry enters outer logging once, a cache hit skips inner layers/body, retry does not reenter outer layers. |
| META-C130, META-C131 | Invalid original arity, duplicate/unknown keyword, block, parameter or default prevents even a cache-hit callback; receiver/operand/default counters and failure precedence remain ordinary; defaults evaluate once; snapshot field Types/order, exact receiver/qualified slot, generic bindings and original/current block identities are observable; snapshot mutation is refused without exposing private state. |
| META-C132, META-C133 | Empty/new with all five optional fields; absent versus explicit nil; constructor input-container snapshot isolation; unknown/duplicate fields and wrong container/key Types reject; fixed slots bind by Symbol name, not index; wrong-channel names reject; rest/keyword-rest replace whole containers; unknown keywords require original keyword rest; full resulting payload checks before inner marker increments; all block absent/required/optional/no-channel cases and invariant Block signatures; successful inner snapshot changes while outer/provenance stays unchanged; nested same-name patches and repeated next calls use the specified incoming basis; no defaults/splats/generic inference rerun. |
| META-C133, META-C134 | Zero/one/sequential multiple attempts, including retry after failure and different changes per attempt; next has one optional positional parameter and no keywords/rest/trailing block; fresh parameter cells and rest containers per layer/body attempt, escaping captures independent, shared object effects retained; invalid change computation is not rolled back. |
| META-C135 | Invalid original-body, inner-wrapper and zero-attempt results fail at their own exact-R boundary before outer observation; ordinary TypeError can be caught and recovered from; Never, nilability, closed generics and setter-result semantics stay exact; callback arity/kind/mode/result annotation mismatch rejects; no Closure covariance or BoundMethod conversion; sync Task-valued results are not awaited. |
| META-C136 | Transform remains synchronous/non-awaiting while registering an async callback; eager prefix and already-complete awaits add no scheduling hop; each invocation has a fresh outer Task, every next a fresh bridge; Task invariance uses real adapters and per-layer R checks; repeat awaits preserve completion/failure-root identity; async setup and replacement failures reach failed Tasks before inner effects; operand failures retain the pre-Task path. |
| META-C137, META-C139 | Same-owner synchronous helper succeeds, owner suspension/resumption succeeds; post-completion, foreign async helper including eager prefix, reentrant and overlapping next calls reject with exact categories and mode-dependent delivery; another attempt is allowed only after prior bridge settlement; early normal completion fails admission, early exceptional completion retains its primary context plus structured diagnostic; unfinished inner work continues, with no implicit cancellation/join and abandoned failures still reportable. |
| META-C138 | Captures/cache persist across normal calls, distinct applications and closed materializations are isolated unless objects are explicitly shared; all five replay reasons rebuild each application once from canonical undecorated artifacts with fresh state; retained Method/BoundMethod keeps old chain/cache; current-owner loss rejects before cache entry; removal/undef and ordinary/qualified slot isolation remain exact; original super uses current MRO; wrapper/block lexical receiver/package and reflection authority do not transfer. |
| META-C128, META-C138, META-C139 | Method wrapping needs method_body, not method_set; Property wrapping needs property_body, not a method_body loophole; deny-set alone permits compatible wrapping, deny-body rejects it; Class/Module addition needs method_set and existing override/impl/visibility/Contract authority; absent accessors and Contract executable changes reject; ordinary, Module/main, class-object, operator, generic, qualified, generated, native-backed and async targets preserve exact contracts; targets runtime-protected against body replacement, otherwise nonreplaceable, or native-unsafe reject without partial publication; ordinary Methods with `protected` visibility remain wrappable subject to existing access and capability checks. |

Informative note: [The v1.35 ledger](../../docs/spec-defects-v1.md#v135-approved-decorator-errata) records the actual corpus audit and V428–V431 fixture mismatches. This table is a coverage requirement, not an implementation or an assertion that new runtime tests pass.

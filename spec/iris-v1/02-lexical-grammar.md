...
IRIS-V1-GRAMMAR-C067: The v1.22 errata admits a CLOSED generic name as a complete expression, so `Box<String>` may be used as a value and not only as the receiver of a `postfix_part`. This supplies the syntax D-456 already presupposes when it makes an interned Type object distinct from the Class object, which IRIS-V1-TYPES-V257 observes by comparing `Box<String>.type` against `Box<String>`. It supersedes in place the `postfix_part` requirement in IRIS-V1-GRAMMAR-C063; every other condition C063 states is unchanged. IRIS-V1-GRAMMAR-C020 is NOT weakened. The generic reading is still taken only where the name is a `type_name` and the bracket pair closes with a `>`, and the closing `>` must now be followed by a `postfix_part` OR by a token that cannot continue an expression: a statement terminator, `,`, `]`, `)`, `}`, or end of input. Where both readings would otherwise be well formed the OPERATOR reading still wins, so `a < b` remains a comparison and `a >> b` remains a right shift. A bare generic name without arguments remains definition metadata under IRIS-V1-TYPES-C061 and is NOT admitted here.
...
IRIS-V1-GRAMMAR-C073: The v1.36 language revision establishes instance field declaration syntax with explicit mutability keywords, adding `instance_field_decl ::= ("let" | "mut") ivar_name type_annotation? ("=" expression)?` and admitting it in Class and Module bodies. A field declared with `let` is immutable after instance initialization, while `mut` declares an assignable instance cell. Declaring an instance field without `let` or `mut` in a class body is rejected with diagnostic `PARSE_ORIGIN_BODY_REQUIRES_DECLARATION`. This explicitly supersedes unadorned field declarations under IRIS-V1-RUNTIME-C061.

IRIS-V1-GRAMMAR-C074: The v1.36 language revision introduces the `%[` array literal opener token (`ARRAY_OPEN`) and requires array literal expressions to use `%[ elements ]`. Superseding IRIS-V1-GRAMMAR-C015 and IRIS-V1-GRAMMAR-C040 in place, the fixed token inventory increases from 69 to 70 names by adding `ARRAY_OPEN` (`%[`). Bare square brackets `[` and `]` (`LBRACKET` and `RBRACKET`) remain reserved exclusively for indexing (`receiver[index]`), slicing, and pattern matching destructuring (`[head, *tail]`). Writing a bare bracket expression `[elements]` in expression position is rejected with diagnostic `PARSE_ARRAY_PREFIX_REQUIRED`.

IRIS-V1-GRAMMAR-C075: The v1.36 language revision replaces the unadorned type-test operator `is` with the predicate operator `is?` (`IS_QUERY`). Superseding IRIS-V1-GRAMMAR-C015 and IRIS-V1-GRAMMAR-C016 in place, the operator spelling `is?` replaces `is` at precedence rank 11 (relational and type operators). The fixed expression operator inventory remains 45 fixed spellings and 47 operator forms, with `is?` taking the slot of `is`. Writing unadorned `is` in an expression is rejected with diagnostic `PARSE_IS_QUESTION_REQUIRED`. `same?` remains a contextual identity selector at named infix precedence rank 13.

IRIS-V1-GRAMMAR-C076: The v1.36 language revision establishes postfix non-null evaluation syntax `expression !` at postfix precedence rank 1. A postfix `!` operator evaluates its receiver expression and asserts non-nullity: if the operand evaluates to `nil`, evaluation terminates by raising `TypeError`. Postfix `!` chains with member access (`x!.member`), indexing (`x![0]`), and calls (`(expr)!()`). A trailing `!` that forms part of an identifier selector, as in `foo!()`, is tokenized as part of selector identity under IRIS-V1-GRAMMAR-C010 and is not rewritten as non-null evaluation; explicit parenthesization `(foo)!()` forces postfix non-null evaluation.

IRIS-V1-GRAMMAR-C077: The v1.36 language revision introduces top-level static `impl_decl ::= "export"? "impl" type_expr "for" type_expr where_clause? "{" method_decl* "}"` declarations in program scope, establishing static Contract conformance under IRIS-V1-TYPES-C100 through IRIS-V1-TYPES-C107. Each `impl` block MUST name exactly one target Class and one target Contract, rejecting multiple targets with `PARSE_IMPL_REQUIRES_ONE_CONTRACT`. The body is requirement-only: it MUST contain only Method declarations without the member-level `impl` modifier, rejecting stored fields with `PARSE_IMPL_METHODS_ONLY` and writing `impl fun` with `PARSE_LEGACY_METHOD_IMPL`. Class header `for` is removed and rejected with `PARSE_LEGACY_CLASS_FOR`.

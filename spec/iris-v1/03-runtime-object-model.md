...
IRIS-V1-RUNTIME-C163: The v1.12 errata defines `Kernel` as the language-core Module composed into `Object`. It carries language-core Type aliases and declarations that must be visible everywhere without an import, and it is an ordinary Module under IRIS-V1-RUNTIME-C046 through IRIS-V1-RUNTIME-C048, so its composition, lookup order, and deduplication follow the same rules as any other Module. `Kernel` MUST NOT supply, replace, or shadow any default behavior that IRIS-V1-RUNTIME-C005 assigns to `Object`, namely default comparison, truthiness, missing-message, and zero-argument `initialize`. Because IRIS-V1-RUNTIME-C046 searches composed Modules BEFORE the runtime superclass, a member in `Kernel` that collided with one of those four would take precedence and defeat `C005`; such a declaration MUST be rejected. `Kernel` is not a second root Class and does not affect the single-root requirement of `C005`.

IRIS-V1-RUNTIME-C164: The v1.13 errata restates the conventional migration signature as `migrate_revision(source: ClassRevision, target: ClassRevision) -> Nil`. The previous spelling named its first parameter `from`, which IRIS-V1-GRAMMAR-C013 reserves as a keyword for `import_decl` and `raise_cause`, so the signature `D-266` stated could not be written in Iris source at all. Only the two parameter NAMES change: the selector, parameter Types, return Type, and every semantic owned by `D-264`, `D-265` and `D-266` are unchanged, including that the Method is an ordinary replaceable Method the runtime never invokes automatically. This clause supersedes the parameter names in IRIS-V1-RUNTIME-C060 and in `D-266`.
...
IRIS-V1-RUNTIME-C165: The v1.36 revision gives every named Class and Module exactly one origin declaration in one package identity. A second non-open origin for the same qualified identity MUST be rejected and MUST NOT merge members, layout, superclass, generic parameters, policy, or initialization order. Compatible structural runtime change remains an operation on the same logical identity through `open class` or `open module`.

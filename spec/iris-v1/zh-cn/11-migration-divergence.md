...
IRIS-V1-MIGRATION-C003：只由 `Document/Iris Programming Language Intro.pdf` 支持的行为 MUST NOT 在没有冻结决策以及遗留脚本、实现行为或显式 v1 替代条款之一佐证时标记为 `preserve`。仅 PDF 证据可以支持 `removed`、`intentional-divergence` 或 `deferred`，但不能支持保留。
...
| `involve` | `Document/Iris Revival Design Review.md:386` | `intentional-divergence` | IRIS-V1-MIG-010 |
| `native` | `Document/Iris Revival Design Review.md:386` and `D-509` | `removed` | IRIS-V1-MIG-005 |
...
IRIS-V1-MIGRATION-C010：本章合并 D-001、D-034、D-145、D-146、D-278、D-362、D-438、D-440、D-441、D-447、D-459、D-460、D-461、D-462、D-463、D-464、D-469、D-509 和 D-510 中要求或解释遗留分歧的内容。它还记录来自 `Document/Iris Revival Design Review.md`、`Document/IrisLangHighLight(for NP++).xml`、`Document/Iris Programming Language Intro.pdf` 和 `Iris Library Test/test script/` 的历史证据，但不赋予这些来源规范性权威。

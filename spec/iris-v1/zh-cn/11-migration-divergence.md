...
| IRIS-V1-MIG-003 | 遗留 `block` 标记以及 `with` 或 `without` 块存在分支 | `legacy/Document/Iris Revival Design Review.md:297`; `legacy/Document/IrisLangHighLight(for NP++).xml:27`; `Iris Library Test/test script/statement/test_block_statement.ir:1`; `Iris Library Test/test script/statement/test_closure_block.ir:11` | `intentional-divergence` | [IRIS-V1-CONTROL-C023](04-bindings-callables-control-flow.md), [IRIS-V1-CONTROL-C030](04-bindings-callables-control-flow.md), [IRIS-V1-GRAMMAR-C050](02-lexical-grammar.md) | IRIS-V1-MIGRATION-EX003：遗留 `block` 加 `with` 或 `without` 变为声明的 `&block: Block<(String) -> Nil> = nil` 参数，以及普通的 `if block != nil { block(value) }`。 | D-423 把块传递变成显式 callable 通道，而不是主体外部标记。 |
| IRIS-V1-MIG-004 | 遗留 `cast.call(...)` 作为尾随块调用 | `legacy/Document/Iris Revival Design Review.md:297`; `legacy/Document/Iris Revival Design Review.md:386`; `Iris Library Test/test script/statement/test_block_statement.ir:5`; `Iris Library Test/test script/expression/test_functioncall_expression.ir:26` | `intentional-divergence` | [IRIS-V1-CONTROL-C030](04-bindings-callables-control-flow.md), [IRIS-V1-TYPES-C029](05-types-contracts-generics.md), [IRIS-V1-TYPES-C030](05-types-contracts-generics.md) | IRIS-V1-MIGRATION-EX004：遗留 `cast.call(value)` 对 `&block` 参数变为 `block.call(value)`；类型转换使用 `value as Type` 或 `value as? Type`。v1.10 勘误依据 IRIS-V1-CONTROL-C076 取代本行的调用部分：`call` 是唯一的调用拼写，因此遗留的 `.call` 调用面被保留而非改为直接应用；移除 `cast` 重载含义的部分继续有效。 | D-454 把 `as` 和 `as?` 恢复为已检查类型操作，并移除 `cast` 的重载含义。 |

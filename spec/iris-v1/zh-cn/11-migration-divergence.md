...
| IRIS-V1-MIG-009 | `implements` 和 `satisfies` 作为可能的符合性词 | `Document/Iris Revival Design Review.md:327`; `spec/drafts/iris-language-specification.md:538` | `removed` | [IRIS-V1-GRAMMAR-C014](02-lexical-grammar.md), [IRIS-V1-TYPES-C044](05-types-contracts-generics.md) | IRIS-V1-MIGRATION-EX009：写 `class Sprite for Drawable { ... }`，不要写 `implements Drawable` 或 `satisfies Drawable`。 | D-278 冻结 `for` 作为 Class 头符合性关键字，D-509 把这两个词排除在保留状态之外。 |

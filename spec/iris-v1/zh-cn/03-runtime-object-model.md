...
| `IRIS-V1-RUNTIME-V015` | 失败  | 保留的 Method 在所有者从当前 MRO 中删除后执行 `super`            | `InvalidSuperError`                                                     |
...
| `IRIS-V1-RUNTIME-V067` | 正向 | 需要解释器；需要 JIT；本地人不适用；原因：纯 Iris 融合算术没有原生边界 | `Float32.from_bits(0x3f800001).mul_add(Float32.from_bits(0x3f800001), -1.0f32).to_bits(); (Float32.from_bits(0x3f800001) * Float32.from_bits(0x3f800001) + -1.0f32).to_bits(); type_of(1.0f32.mul_add(2, 3.0f64))` | 第一个Integer是一次舍入融合的二进制32位；第二个是独立舍入的乘法加法位和不同；最终Type是`Float64`。 | `D-056`, `D-058`, `D-059` |
...
| `IRIS-V1-RUNTIME-V077` | 正向 | 需要解释器；需要 JIT；原生不适用 | `obj.method == obj.method; closure() == closure(); obj.method same? obj.method`|每个独立评估的可调用对象比较不相等且不相同。 | `D-107`, `D-108` |

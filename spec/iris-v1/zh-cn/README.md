...
Informative note: 以下 C019 至 C022 补入英文索引中既有修订程序的缺失译文，不修改已发布英文条款。它们使上方 C001 的禁令受正式批准勘误程序限定；符合该程序的补全不是静默补全。矩阵原有的同名 TRACE-C019 是既存 ID 冲突，引用修订程序时以本 README 为准，不重新编号历史条款。

IRIS-V1-TRACE-C019: 实现经验 MAY 揭示冻结章节规定了语义要求却未提供满足它的具体源语法，或遗漏了实现无法避免决定的规则。这是勘误候选，不是发明语言特性的许可。作者在取得已记录的所有者批准前 MUST NOT 对勘误候选采取行动。

IRIS-V1-TRACE-C020: 获批勘误修订 MUST 将规范修订递增到 `v1.1` 及后续 `v1.n`，MUST 依据 IRIS-V1-TRACE-C007 保留每个已发布条款、示例和向量 ID，并 MUST 使用下一个可用条款编号添加新材料。重新编号或删除已发布 ID 仍被禁止。英文各章和 `zh-cn/` 下的简体中文翻译 MUST 在同一修订中更新，以保持一致。

IRIS-V1-TRACE-C021: 勘误修订 MAY 补全已陈述但未规定的空缺，为另一条款已要求的行为补充缺失语法产生式，或记录实现否则必须猜测的推断规则。它 MUST NOT 重新解释已决定的语义、弱化冻结保证或移除要求。修订若扩展先前已关闭的清单，例如 D-509 固定的保留关键字集合，修订条款 MUST 明确说明新总数并就地取代旧计数。

IRIS-V1-TRACE-C022: 每次勘误修订在发布前 MUST 对照已提交一致性语料库验证。修订作者 MUST 报告结果改变的任何向量，且 MUST NOT 调整、重新标记或删除向量来掩盖修订引入的行为变更。依据修订语义合理失败的向量是应报告的证据，不是应隐藏的缺陷。

...
IRIS-V1-TRACE-C023: The v1.35 decorator errata is authorized by the owner's recorded instruction `按照建议开始`, approving the complete recommendation table in the [approval record](../../docs/decorator-capability-boundaries-proposal.md#approved-v135-checklist). It adds IRIS-V1-META-C127 through IRIS-V1-META-C139 in [chapter 08](08-modules-metaprogramming.md#v135-decorator-invocation-protocol) and IRIS-V1-CONFORMANCE-C076 in [chapter 12](12-conformance.md#v135-decorator-coverage-obligations). The owner explicitly authorizes a bounded exception to this README's IRIS-V1-TRACE-C021: META-C139 supersedes only META-C125's unconditional static rejection phase for a wrong-kind Transformation, retaining static rejection when provable and otherwise requiring runtime candidate validation, `IRIS-DECORATOR-KIND`, and complete rollback before publication. Writers MUST NOT generalize this exception to any other frozen guarantee or rewrite the published clause/vector text. All other additions close the approved invocation, operation and lifecycle gaps without adding keywords or callable runtime kinds; the 14-artifact inventory is unchanged. The [defect ledger](../../docs/spec-defects-v1.md#v135-approved-decorator-errata) records corpus impact and audit evidence. Documentation publication MUST NOT be represented as runtime support or a passing conformance freeze: uncovered new clauses and existing corpus defects remain reportable under C022 and CONFORMANCE-C057.

IRIS-V1-TRACE-C024: The v1.36 language revision is authorized by explicit owner approval as recorded in the defect ledger, establishing an authorized bounded breaking revision within language major 1. Under this approval, breaking syntax adjustments are permitted to establish coherent v1.36 foundations: top-level static `impl Class for Contract` declarations replace class header `for` and member-level `impl fun`; origin class and module bodies are purely declarative; dynamic structural modification is restricted to `open` blocks; instance fields require explicit mutability markers `let @field` or `mut @field`; array literals require the `%[]` prefix to disambiguate from indexing and patterns; `is?` replaces unadorned `is` as the type query operator; and postfix `!` provides non-null evaluation alongside `foo!` selector disambiguation. These changes supersede earlier transitional rules in place while preserving the 14-artifact specification inventory. Conformance and migration obligations are governed by IRIS-V1-CONFORMANCE-C077, IRIS-V1-CONFORMANCE-C078, and IRIS-V1-MIGRATION-C013.

IRIS-V1-TRACE-C025: The v1.37 safe navigation errata is authorized by the owner's recorded instruction "按照你的推荐来", approving the safe member navigation design under the revision procedure in IRIS-V1-TRACE-C019 through IRIS-V1-TRACE-C022. It adds IRIS-V1-GRAMMAR-C080 through IRIS-V1-GRAMMAR-C084 in chapter 02, IRIS-V1-RUNTIME-C168 through IRIS-V1-RUNTIME-C170 in chapter 03, IRIS-V1-CONTROL-C084 through IRIS-V1-CONTROL-C086 in chapter 04, and IRIS-V1-CONFORMANCE-C079 through IRIS-V1-CONFORMANCE-C080 in chapter 12, with matching Chinese translations. The errata defines `?.` as an atomic token distinct from selector suffix `?`, establishes safe member-only navigation with nil-only short-circuit evaluation, evaluates receiver expressions exactly once, skips downstream member access, call argument evaluation, indexing, and blocks upon nil, preserves `ready?` and `save!` selector identities, retains `(expr)!` non-null evaluation, excludes `?.(...)`, `?[...]`, and `.!`, and maps the committed conformance evidence `IRIS-V1-GRAMMAR-V908` through `IRIS-V1-GRAMMAR-V910`, `IRIS-V1-RUNTIME-V986`, and `IRIS-V1-CONFORMANCE-V938`. Published vector rows and previous semantics remain preserved.

...
IRIS-V1-TYPES-C097: The v1.21 errata fixes the diagnostic a FAILED per-closed materialization raises. IRIS-V1-TYPES-C066 requires a failed closed-Class materialization to discard candidate state and publish nothing, but names no diagnostic, while IRIS-V1-TYPES-V241 already requires `TypeContractError`. An exception escaping a per-closed class property initializer is therefore reported as `TypeContractError`, matching how IRIS-V1-TYPES-C067 reports a constraint failure on the same materialization path. The originating exception is carried as the cause, so the initializer's own error remains observable. This supplies a rule an implementation cannot avoid deciding; it does not change what C066 discards, and C066's refusal to undo external side effects is unchanged.

IRIS-V1-TYPES-C098: The v1.29 errata fixes the ancestry bound IRIS-V1-TYPES-C045 and `D-174` protect. A transactional runtime-superclass change MUST be rejected with `TypeContractError` before publication when the proposed ancestry drops a declared ancestor that carries a static spine fact in the `D-173` sense: a declared Contract, or a contract-visible member name and signature the target relies on. A declared ancestor carrying no such fact is NOT protected by this clause, and dropping it publishes normally; the consequences for a Method retained from that ancestor are owned by `D-104` and IRIS-V1-RUNTIME-C015, which raise `MethodBindingError` at reflective invocation entry rather than refusing the ancestry change. Narrowing ancestry by inserting a Class that still reaches every protected declared ancestor is permitted, since it preserves every static subtype assumption. The permanent declared bound `D-262` describes is unchanged: this clause fixes WHICH declared ancestors the bound protects, and reverses no published outcome.

IRIS-V1-TYPES-C099: The v1.30 errata fixes the spelling that ATTEMPTS to remove a declared Contract from a Class. A transaction candidate MAY attempt it as `remove_contract(contract)`, the reflective form being `Reflection::Class.remove_contract(target, contract)`, which IRIS-V1-META-C119 makes ONE implementation with the Class-level entry point exactly as it does for `set_superclass`. The attempt is a candidate mutation under IRIS-V1-META-C023, so it targets the CURRENT transaction candidate. IRIS-V1-TYPES-C045 makes declared Contract conformance IMMUTABLE for a revision's static spine, so the attempt MUST be rejected with `TypeContractError` BEFORE publication, and the target MUST retain its declared Contract set, its active revision, and its conformance. This clause supplies only the SPELLING the refusal needs in order to be observable; it grants no ability to remove a declared Contract and reverses no published outcome. A different declared Contract set requires a distinct Class revision or declaration through the explicit class-evolution system, exactly as `D-173` already states.

IRIS-V1-TYPES-C100: The v1.36 errata introduces top-level static `impl Class for Contract` declarations and explicitly supersedes the header `for` class conformance syntax in IRIS-V1-TYPES-C044 and the member-level `impl` modifier syntax in IRIS-V1-TYPES-C046. A nominal Class MUST NOT declare Contract conformance in its class header, and writing `class Name for Contract` MUST be rejected with diagnostic `PARSE_LEGACY_CLASS_FOR`. Contract conformance is established statically and exclusively by top-level `impl TargetClass for TargetContract { ... }` declarations. Each `impl` block MUST name exactly one target Class and exactly one target Contract. An attempt to declare conformance to multiple Contracts in a single `impl` block, such as `impl Box for Show, Hashable`, MUST be rejected at parse time with diagnostic `PARSE_IMPL_REQUIRES_ONE_CONTRACT`.

IRIS-V1-TYPES-C101: Top-level static `impl Class for Contract` declarations are order-independent. An `impl` block MAY appear before, between, or after the declarations of its target Class and target Contract within a compilation unit. Static resolution collects all top-level `impl` declarations ahead of execution and validates nominal conformance prior to running origin bodies.

IRIS-V1-TYPES-C102: The body of an `impl Class for Contract` declaration is requirement-only. It MUST contain only instance Method declarations corresponding directly to requirements declared by the target Contract or its inherited parents. It MUST NOT contain stored fields, raw instance variables, initializers, class variables, executable statements, nested types, or extraneous methods unrequired by the Contract. Writing fields inside an `impl` block MUST be rejected with diagnostic `PARSE_IMPL_METHODS_ONLY`. Writing an extraneous method not present in the Contract requirements MUST be rejected with `TypeContractError`. Methods declared within an `impl` block MUST NOT carry the member-level `impl` modifier; writing `impl fun` inside an `impl` block or a class body MUST be rejected with diagnostic `PARSE_LEGACY_METHOD_IMPL`.

IRIS-V1-TYPES-C103: Methods declared within an `impl Class for Contract` block populate both ordinary dispatch and qualified Contract slot dispatch for the target Class. A method declared as `public fun m()` in the `impl` block installs an ordinary slot reachable via `receiver.m()` and a qualified slot reachable via `(receiver as TargetContract)..m()`. If an implementing method is declared as `private fun m()`, ordinary dispatch is restricted according to visibility while qualified dispatch via the Contract view remains accessible to authorized callers. Both surfaces reflect the single implementing method identity.

IRIS-V1-TYPES-C104: An empty `impl Class for Contract {}` declaration is valid if and only if the target Class already defines or inherits compatible methods for all requirements of the target Contract. The empty `impl` reuses the existing compatible class methods to satisfy the Contract obligations and installs corresponding qualified Contract slots without duplicating method bodies. If any requirement is missing or incompatible, the empty `impl` MUST be rejected with `TypeContractError`.

IRIS-V1-TYPES-C105: Conformance obligations across compilation units enforce uniqueness, coherence, orphan prevention, and non-overlapping implementation rules. Within a program, there MUST be at most one `impl Class for Contract` declaration for any given `(Class, Contract)` pair. Duplicate `impl` declarations for the same Class and Contract MUST be rejected with `TypeContractError`. An `impl` declaration MUST be located in the package defining the target Class or the package defining the target Contract (orphan rule). Two distinct `impl` declarations MUST NOT provide conflicting or overlapping implementations for the same target Class and Contract. Target Class and target Contract MUST exist; specifying an unknown Class or unknown Contract MUST be rejected with `TypeContractError`.

IRIS-V1-TYPES-C106: Top-level `impl` declarations apply to the generic definition of a generic Class, as in `impl Box<T> for Show where T: Object { ... }`. Closed generic specialization is prohibited in Iris v1: source syntax MUST NOT declare `impl Box<Integer> for Show` or provide closed-specialized implementation bodies. All closed generic constructions share the single static implementation defined on the generic definition.

IRIS-V1-TYPES-C107: The set of Contracts implemented by a Class is fixed statically before program execution and is immutable at run time. Runtime metaprogramming, open class transactions, dynamic reflection, and native extensions MUST NOT add, remove, or replace nominal Contract conformance on an existing Class. Dynamic open transactions on a Class revalidate the full static `impl` obligations against the active candidate; any modification that breaks an established `impl` obligation MUST be rejected with `TypeContractError` and roll back atomically.

IRIS-V1-TYPES-C108: Modules MUST NOT declare Contract conformance with `for` and MUST NOT be targets of top-level `impl Module for Contract`. A Module MAY declare constraints on its implementing receiver using `module Name where Self: Contract { ... }`. When a Module with a `where Self: Contract` constraint is composed into a Class via `mixin`, the composing Class MUST statically satisfy the `Contract` constraint through an `impl Class for Contract` declaration; otherwise, candidate validation MUST fail with `TypeContractError`.

IRIS-V1-TYPES-C109: Contract views remain immutable identity-less capability values under IRIS-V1-TYPES-C050. Equality `==` and hashing `hash` on Contract views derive strictly from receiver identity/equality and Contract identity per IRIS-V1-TYPES-C050 and IRIS-V1-TYPES-C051. Testing identity with `same?` on any Contract view MUST raise `IdentityError`. This identity-less capability model and error behavior remain unchanged in Iris v1.36.

...
bool IrisInterpreter::RegistClass(const string& strClassFullFieldName, IIrisClass* pClass, bool bNative) {
	...
	if (bNative) {
		...
		pClass->m_pInternClass->m_pExternClass = pClass;
	}
...
{
	IR_ExtentionInitializeStruct eisStruct;
	eisStruct.m_pInterpreter = this;
	eisStruct.m_pGC = IrisGC::CurrentGC();
	eisStruct.m_pMemoryPool = IrisAbstractMemoryPool::GetCurrentMemoryPool();
	eisStruct.m_pMemoryPoolList = IrisAbstractMemoryPool::GetMemoryPoolList();
	eisStruct.m_pThreadManager = IrisThreadManager::CurrentThreadManager();
	eisStruct.m_pCompiler = IrisCompiler::CurrentCompiler();
	eisStruct.m_pFatalErrorHandler = IrisFatalErrorHandler::CurrentFatalHandler();
	...
	typedef int(*InitFunc)(PIR_ExtentionInitializeStruct);

	InitFunc pfInitFunc = (InitFunc)GetProcAddress(hMod, "IR_Initialize");
	if (!pfInitFunc) {
		return false;
	}
	if (!pfInitFunc(&eisStruct)) {
		return false;
	}
...
bool IrisInterpreter::load(vector<IR_WORD>& vcVector, unsigned int& nCodePointer, unsigned int nLineNumber, unsigned int nCurrentFileIndex)
{
	...
	case IrisAMType::ImmediateString:
	{
		...
		pInfo->m_ivResultRegister = IrisDev_CreateInstanceByInstantValue(strString.c_str());
	}
		break;
	case IrisAMType::ImmediateUniqueString:
	{
		...
		pInfo->m_ivResultRegister = IrisDev_CreateUniqueStringInstanceByUniqueIndex(iaAM.m_dwIndex);
	}
		break;
	case IrisAMType::ImmediateInteger: 
	{
		...
		pInfo->m_ivResultRegister = IrisDev_CreateInstanceByInstantValue(nInteger);
	}
		break;
	case IrisAMType::ImmediateFloat:
	{
		...
		pInfo->m_ivResultRegister = IrisDev_CreateInstanceByInstantValue(dFloat);
	}
		break;
		...
		// Runtime
			...
			if (pObject) {
				if (pObject->GetClass()->GetInternClass()->IsClassClass()) {
					pInfo->m_ivResultRegister = (IrisDev_GetNativePointer<IrisClassBaseTag*>(pObject))->GetClass()->SearchClassVariable(strIdentifier, bResult);
					if (!bResult) {
						IrisDev_GetNativePointer<IrisClassBaseTag*>(pObject)->GetClass()->AddClassVariable(strIdentifier, m_ivNil);
					}
				}
				else if (pObject->GetClass()->GetInternClass()->IsModuleClass()) {
					pInfo->m_ivResultRegister = IrisDev_GetNativePointer<IrisModuleBaseTag*>(pObject)->GetModule()->SearchClassVariable(strIdentifier, bResult);
					if (!bResult) {
						IrisDev_GetNativePointer<IrisModuleBaseTag*>(pObject)->GetModule()->AddClassVariable(strIdentifier, m_ivNil);
					}
				}
				else {
	...
	case IrisAMType::IndexValue:
	{
		...
		pInfo->m_ivResultRegister = static_cast<IrisObject*>(pInfo->m_ivResultRegister.GetIrisObject())->CallInstanceFunction("[]", pInfo->m_pEnvrionmentRegister, nullptr, CallerSide::Outside, nLineNumber, nCurrentFileIndex);
	}
		break;
...
bool IrisInterpreter::nol_call(vector<IR_WORD>& vcVector, unsigned int& nCodePointer, unsigned int nLineNumber, unsigned int nCurrentFileIndex)
{
	...
	if (pInfo->m_ivResultRegister.GetIrisObject()->GetClass()->GetInternClass()->IsClassClass()) {
		auto pNative = IrisDev_GetNativePointer<IrisClassBaseTag*>(pInfo->m_ivResultRegister)->GetClass();
		pInfo->m_ivResultRegister = pNative->CallClassMethod(strMethodName, pInfo->m_pEnvrionmentRegister, &ivsValues, CallerSide::Outside, nLineNumber, nCurrentFileIndex);
	}
	else if (pInfo->m_ivResultRegister.GetIrisObject()->GetClass()->GetInternClass()->IsModuleClass()) {
		pInfo->m_ivResultRegister = IrisDev_GetNativePointer<IrisModuleBaseTag*>(pInfo->m_ivResultRegister)->GetModule()->CallClassMethod(strMethodName, pInfo->m_pEnvrionmentRegister, &ivsValues, CallerSide::Outside, nLineNumber, nCurrentFileIndex);
	}
	else {
		pInfo->m_stStack.Push(pInfo->m_ivResultRegister);
...
bool IrisInterpreter::fld_load(vector<IR_WORD>& vcVector, unsigned int& nCodePointer, unsigned int nLineNumber, unsigned int nCurrentFileIndex)
{
	...
	if (!pInfo->m_lsFieldsRegister.empty()) {
		...
		auto pModule = IrisDev_GetNativePointer<IrisModuleBaseTag*>(ivResultRegister)->GetModule();
		...
		for (auto iField = pInfo->m_lsFieldsRegister.begin(); iField != pInfo->m_lsFieldsRegister.end(); ++iField) {
			...
			if (pRouterObject->GetClass()->GetInternClass()->IsModuleClass()) {
				pModule = IrisDev_GetNativePointer<IrisModuleBaseTag*>((IrisValue&)ivResult)->GetModule();
		...
		if (pSearchObject->GetClass()->GetInternClass()->IsClassClass()) {
			auto pClass = IrisDev_GetNativePointer<IrisClassBaseTag*>(ivSearchResult)->GetClass();
			pInfo->m_ivResultRegister = pClass->SearchConstance(m_pCurrentCompiler->GetIdentifier(nTargetIndex, nCurrentFileIndex), bResult);
		}
		else {
			auto pModule = IrisDev_GetNativePointer<IrisModuleBaseTag*>(ivSearchResult)->GetModule();
		...
		if (pObject->GetClass()->GetInternClass()->IsModuleClass()) {
			...
			auto pClass = IrisDev_GetNativePointer<IrisClassBaseTag*>(ivResultRegister)->GetClass();
			pInfo->m_ivResultRegister = pClass->SearchConstance(m_pCurrentCompiler->GetIdentifier(nTargetIndex, nCurrentFileIndex), bResult);
		}
	}
	else {
...
bool IrisInterpreter::load_self(vector<IR_WORD>& vcVector, unsigned int& nCodePointer, unsigned int nLineNumber, unsigned int nCurrentFileIndex)
{
	...
	//	|| pInfo->m_pEnvrionmentRegister->m_bIsClosure) {
	//	// ** Error **
	//	IrisFatalErrorHandler::CurrentFatalHandler()->ShowFatalErrorMessage(IrisFatalErrorHandler::FatalErrorType::SelfPointerIrregular, nLineNumber, nFileIndex,
	//		"Statement of self must be used in INSTANCE METHOD OF CLASS.");
	//	return false;
	//}
	...
	//	pInfo->m_pEnvrionmentRegister->m_pClosureBlock = pInfo->m_pClosureBlockRegister;
	//	pInfo->m_pClosureBlockRegister = nullptr;
	//	IrisContextEnvironment* pTmpEnv = pInfo->m_pEnvrionmentRegister->m_pClosureBlock->GetCurrentContextEnvironment();
	//	while (pTmpEnv) {
	//		++pTmpEnv->m_nReferenced;
	//		pTmpEnv = pTmpEnv->m_pUpperContextEnvironment;
	//	}
	...
	//if (pObject->GetClass()->IsClassClass() || pObject->GetClass()->IsModuleClass()) {
	//	// ** Error **
	//	IrisFatalErrorHandler::CurrentFatalHandler()->ShowFatalErrorMessage(IrisFatalErrorHandler::FatalErrorType::SelfPointerIrregular, nLineNumber, nFileIndex,
	//		"Statement of self must be used in INSTANCE METHOD OF CLASS.");
	//	return false;
	...
	//IrisAM iaAM = GetOneAM(vcVector, nCodePointer);
	//const string& strMethodName = m_pCurrentCompiler->GetIdentifier(iaAM.m_dwIndex, nFileIndex);
	...
	//	//	ivsValues[nParameters - i - 1] = *it;
	//	//}
	//	ivsValues.assign(pInfo->m_stStack.m_lsStack.begin() + pInfo->m_stStack.m_lsStack.size() - nParameters, pInfo->m_stStack.m_lsStack.end());
	//	pValues = &ivsValues;
	//}
...
bool IrisInterpreter::ini_cnt(vector<IR_WORD>& vcVector, unsigned int& nCodePointer, unsigned int nLineNumber, unsigned int nCurrentFileIndex)
{
	...
	pInfo->m_ivCounterRegister = IrisDev_CreateInstanceByInstantValue(0);
	return true;
}
...
bool IrisInterpreter::cmp_tac(vector<IR_WORD>& vcVector, unsigned int& nCodePointer, unsigned int nLineNumber, unsigned int nCurrentFileIndex)
{
	...
	if (pInfo->m_bUnimitedLoopFlagRegister) {
		...
		pInfo->m_ivResultRegister = 
			IrisDev_GetNativePointer<IrisIntegerTag*>(pInfo->m_ivCounterRegister)->m_nInteger < IrisDev_GetNativePointer<IrisIntegerTag*>(pInfo->m_ivTimerRegister)->m_nInteger
			? m_ivTrue
			: m_ivFalse;
	}
	return true;
...
{
	auto pInfo = IrisDev_GetCurrentThreadInfo();
	++IrisDev_GetNativePointer<IrisIntegerTag*>(pInfo->m_ivCounterRegister)->m_nInteger;
	return true;
}
...
bool IrisInterpreter::def_cls(vector<IR_WORD>& vcVector, unsigned int& nCodePointer, unsigned int nLineNumber, unsigned int nCurrentFileIndex)
{
	...
	if (!pClass) {
		pClass = new IrisClass(strClassName, nullptr, pUpperEnvironment ? pUpperEnvironment->m_uType.m_pModule : nullptr);
		pClass->m_pExternClass = new IrisDummyClass();
...
bool IrisInterpreter::add_ext(vector<IR_WORD>& vcVector, unsigned int& nCodePointer, unsigned int nLineNumber, unsigned int nCurrentFileIndex)
{
	...
	IrisClass* pClass = pInfo->m_pEnvrionmentRegister->m_uType.m_pClass;
	pClass->SetSuperClass(IrisDev_GetNativePointer<IrisClassBaseTag*>(pInfo->m_ivResultRegister)->GetClass());
...
bool IrisInterpreter::spr(vector<IR_WORD>& vcVector, unsigned int& nCodePointer, unsigned int nLineNumber, unsigned int nCurrentFileIndex)
{
	...
	if (!pObject || !pObject->GetClass()->GetInternClass()->IsNormalClass()) {
	...
	if (pMethod) {
		...
		if (nParameters > 0) {
			pInfo->m_ivResultRegister = pMethod->Call(ivObj, pInfo->m_pEnvrionmentRegister, &ivsValues);
		}
		else {
			pInfo->m_ivResultRegister = pMethod->Call(ivObj, pInfo->m_pEnvrionmentRegister, nullptr);
		}

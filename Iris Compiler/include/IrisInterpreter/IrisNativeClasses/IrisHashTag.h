...
private:
	struct IrisValueHash {
		size_t operator () (const IrisValue& ivValue) const {
			...
			size_t nHash = 0;
			if (IrisDev_CheckClass((IrisValue&)ivValue ,"Integer")) {
				nHash = size_t(IrisDev_GetInt((IrisValue&)ivValue));
			}
			else if (IrisDev_CheckClass((IrisValue&)ivValue, "Float")) {
				int e = 0;
				double dValue = IrisDev_GetFloat((IrisValue&)ivValue);
				double tmp = dValue;
				if (dValue<0)
				{
					tmp = -dValue;
			...
			else if (IrisDev_CheckClass((IrisValue&)ivValue, "String")) {
	...
	struct IrisValueCmp {
		bool operator()(const IrisValue& ivValue1, const IrisValue& ivValue2) const {
			...
			return IrisDev_CallMethod((IrisValue&)ivValue1, &ivValues, "==") == IrisDev_True();
		}
	};

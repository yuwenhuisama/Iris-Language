...
```
RUNTIME  passed: 71, failed: 3, needs_subsystem: 14, no_fixture: 18, differential: 3   (109 records, buckets sum 109)
GRAMMAR  passed: 26, failed: 0, deferred: 1, authored_expect: 5, unrunnable_source: 9  (41 records)
```

Milestone 2 opened at RUNTIME 41 and closed at 71.
...
Milestone 2 is closed as delivered at 71 of the 74 runnable RUNTIME vectors. None of the three
...
`docs/spec-defects-v1.md` holds 35 rows, 18 resolved, and records the blocker for every one.
...
**Milestone 2 is closed as delivered.** RUNTIME conformance advanced from 41 to 71 across the
...
1. **Qualified Contract dispatch** is the remaining type-system work. Contract objects now exist,
   which closed `V078` and `V080`, but `V081` needs `class ... for C` conformance plus `impl`
   members and the `(value as C)..m()` view call, and `V107` needs Dynamic return-Contract
   enforcement. `V086` needs only the `Dynamic<Object>` slot Type, its behavioural half already
   being correct.

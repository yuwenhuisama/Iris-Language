...
class. A class DECLARING its own keeps that one, since the declared entry is
published after the implicit one and replaces it.
...
happened to capture crashed the machine outright rather than answering
anything at all.

A SUSPENSION is a registered CONTINUATION, not an exit. `C013` leaves the
protected region unentered, so a `using` resource stays open - closing on the
way out ran cleanup a second time on the replayed re-entry, which is the double
close `V084` forbids.
...
the origin declared - so a replacement that no longer fits leaves the
...
TWO YIELDS order by the values they CARRY. The inner `<=>` decides, and `C015`
still holds the comparison to the ordering contract: an inner answer that is
not -1, 0 or 1 breaks it. Answering nil let a carried value with NO order pass
as though the signals themselves were unordered.
...
spellings the language refuses outright.
...
as its own, and the refusal is catchable like any other named failure.
...
reporting an arity failure where the language names a limit.
...
Answering nothing at all made `==` on a bound method a missing message rather
than the `false` the language states.
...
probe on REPRESENTATION, not part of the Array surface the specification
...
recognised while lowering the call rather than evaluated as a name.
...
answers the ATOMS it was built from, so an inner union is named rather than
flattened away.

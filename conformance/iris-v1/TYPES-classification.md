...
| `V206` | diagnostic | executable | `IRIS-V1-META-C022` validates the COMPLETE candidate before publishing and publishes NOTHING from a failed one; `IRIS-V1-TYPES-C006` reports the Contract failure as `TypeContractError`. The member staged before the incompatible one is discarded with it. |
| `V207` | positive | executable | `IRIS-V1-META-C035` lets the block read its OWN candidate after writes while outside code sees the published revision, and `C034` rolls back so the staged property publishes not at all. |
| `V203` | diagnostic | executable | `IRIS-V1-TYPES-C045` forbids an open from incompatibly replacing a declared Contract fact, so a candidate rewriting the Contract-visible RETURN Type is rejected before commit with `TypeContractError`. |
...
| `V241` | diagnostic | executable | The v1.21 errata `IRIS-V1-TYPES-C097` reports an exception escaping a per-closed initializer as `TypeContractError`; `C066` publishes nothing for that construction. `IRIS-V1-CONTROL-C056` now makes a specification-named error catchable, so the row observes BOTH requests raising and the external log keeping `[attempt, attempt]`. |
| `V257` | positive | executable | The v1.22 errata `IRIS-V1-GRAMMAR-C067` admits a bare closed generic name as a complete expression, which is what lets `D-456`'s interned Type object be compared against the Class object. |
...
| --- | --- | --- |
| Metadata fixtures | `V200`, `V202`, `V208`, `V234`, `V260`, `V262` | Re-probed after the candidate model landed, which closed `V203` and `V206`; these five stay blocked on capabilities the transaction alone does not supply. `V200` needs a candidate to REMOVE a declared Contract and no removal API exists. `V202` needs Module composition to carry a conflicting requirement into the candidate.  `V208` has programmatic `A.open` and `define_method` now, and its REFLECTIVE half works, but it also requires the STATIC call to be rejected and static member-existence checking does not exist at all: an ordinary `A.new().nothing()` is equally undiagnosed. `V234` needs opening a GENERIC definition and re-validating every interned closed construction. The row supplies a PROSE metadata schedule, not source: opening a candidate, staging members, then forcing validation failure. This is the same out-of-band scheduling the RUNTIME chapter records as `no-fixture`. The blocker is the METAPROGRAMMING chapter, not this one. `IRIS-V1-META-C001` gives chapter 08 ownership of "declarative and programmatic open transactions, candidate isolation, safepoint commit", and `META-C022` through `META-C025` define the candidate transaction these rows drive: `A.open() { |target| ... }` with `target.define_method`, where structural meta messages target the current candidate and success validates before publication. Neither `open` nor `define_method` exists as a source-level API yet, so these rows cannot be written at all until chapter 08 is opened. `CandidateRevision` and atomic `publish` already exist in the runtime; what is missing is the source-level entry point and the staging that `V206` observes when it requires reflection to expose NEITHER staged member. `V239` additionally needs a Module INITIALIZATION STATUS: a Module body's `raise` and its class-level properties now work, but a raise propagates and halts the program, so `status: not_published` and "neither shared property is observable" cannot be observed from source. `V201` additionally needs the STATIC spine separated from the RUNTIME superclass: the committed `IRIS-V1-RUNTIME-V014` changes a runtime superclass to an unrelated Class and expects that to succeed, so rejecting every superclass change would contradict it. | C)` into three peers, so the compact form `V016` requires is not represented and reflection cannot report the two members the row names. `V016` passes regardless because an intersection and a union are distinct forms whatever their members. |
| Transactional superclass change | `V201` | `D-174` makes the DECLARED superclass an immutable nominal subtype fact and a minimum static contract, so a transactional runtime-superclass change must preserve every static subtype assumption and then publish atomically or fail. Re-probed after the candidate model landed. Declarative `open class Dog extends Object` is rejected as an unsupported construct rather than validated to `TypeContractError`, and the reflective `Reflection::Class.set_superclass` path silently WEAKENS the declared fact: `Dog.type.subtype?(Animal.type)` becomes `false`. Rejecting the reflective path outright is not available, because frozen `IRIS-V1-RUNTIME-V014` REQUIRES that same call to succeed so a retained Method fails to rebind. The row needs the declared static spine kept separate from the runtime superclass, which `D-174` describes and this evaluator does not model. |
| Module load attempts | `V239` | `D-212` aborts the initialization transaction and `V239` requires status `not_published`. That spelling appears EXACTLY ONCE in the specification and no clause defines it as an observable; `IRIS-V1-META-C018` marks a failed Module failed FOR THAT LOAD ATTEMPT and requires an explicit package reload, both chapter 08 package concepts. `IRIS-V1-CONTROL-C012` also makes a Module body top-level executable code, so its `raise` halts the program and no later statement can observe the absence. A Module's class-level property is now READABLE through the Module name, which was the missing capability; see `docs/spec-defects-v1.md`. |

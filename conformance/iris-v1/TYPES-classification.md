...
rows and 16 entries in the Type-normalization law tables. 47 are committed here;
the remaining 33 are NOT yet transcribed and are therefore not covered by any
...
| Generic Types in expression position | `V219`, `V229`, `V246`, `V247`, `V256`, `V257` | The v1.17 errata (`IRIS-V1-GRAMMAR-C063`) added `closed_generic_name` to `primary_expr`, so `Box<String>.new()` now parses and `V245` closed. These rows each need a FURTHER capability: generic inference, Dynamic entry, Module generic arguments, or recursive alias detection. |

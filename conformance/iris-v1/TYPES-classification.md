...
| `V241` | diagnostic | executable | The v1.21 errata `IRIS-V1-TYPES-C097` reports an exception escaping a per-closed initializer as `TypeContractError`; `C066` publishes nothing for that construction. The twice-attempted external log is not observable because `TypeContractError` halts the program, which `docs/spec-defects-v1.md` records. |

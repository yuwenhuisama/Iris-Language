...
rows and 16 entries in the Type-normalization law tables. 56 are committed here;
the remaining 24 are NOT yet transcribed and are therefore not covered by any
...
| String plus another missing capability | `V205`, `V222`, `V228`, `V230` | String IS now a runtime value, which closed `V211` and `V255`. Each row here needs a SECOND capability as well: a Contract declaration, a generic Method or Type in expression position, or an open-Class redeclaration. Re-probed individually rather than assumed. |

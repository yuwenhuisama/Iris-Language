...
| --- | --- | --- |
| `V350` | negative | executable | `D-431` makes a global's identity `(package_id, $name)` with NO flat cross-package namespace. `IRIS-V1-META-C003` and `IRIS-V1-IDENTITY-C030` supply the runtime-local package identity a manifestless script runs under, which is enough to observe the distinction without a manifest. |
| `V351` | positive | executable | `D-432` orders unqualified resolution as LEXICAL scope, then the CURRENT module's declarations, then explicit imports. A Module body `const` belongs to its declaring Module rather than to the shared lexical scope, and an import binds into its own tier, so `[9, 1, 5]` observes the three tiers in one program. |
| Runtime metaprogramming | `V286` | Needs TWO capabilities that do not exist. `ExceptionContext` is not a nameable, reopenable Class, so its public `suppressed` getter cannot be replaced at all: `D-143` makes that replacement the row's premise. And the row observes the UNCAUGHT DIAGNOSTIC PAYLOAD, a reporting surface `D-143` keeps reading the protected record while the public getter returns `[]`; the runner observes a value or an error and has no such channel. |

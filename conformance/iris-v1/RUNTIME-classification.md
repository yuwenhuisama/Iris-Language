...
| `IRIS-V1-RUNTIME-V081` | `:601` | positive | needs-subsystem | Re-probed in milestone 2 and NARROWED. Contract declarations now parse and register, and `class ... for C` parses. What remains is `impl` members and the `(value as C)..m()` qualified view call, so ordinary and qualified namespaces can be shown separate. |
...
| `IRIS-V1-RUNTIME-V086` | `:606` | positive | needs-subsystem | Re-probed in milestone 2 and NARROWED. Per-instance slot independence and the value-type change are both correct through property accessors. Only the `Dynamic<Object>` slot Type assertion remains, which needs the Dynamic type constructor. |

...
| `IRIS-V1-RUNTIME-V013` | `:528`, `:643` | positive | no-fixture | Re-probed in milestone 2 and RECLASSIFIED. The row supplies a prose SCHEDULE, not source: it requires entering a frame, committing a replacement while that frame is suspended, then resuming it. That is the same out-of-band scheduling `V025` waits on. It had been tagged executable and was counted as a failure. |
...
| `IRIS-V1-RUNTIME-V079` | `:599` | positive | needs-subsystem | Re-probed in milestone 2 and NARROWED. The identity hash `D-111` and `IRIS-V1-RUNTIME-C088` require now EXISTS: an ordinary object answers `hash` with a runtime-stable value assigned at allocation, the same object keeps it across later allocations, distinct objects differ, and no address is exposed. What the row still needs is the `compact_gc` step its fixture names, since a compacting GC does not exist, so the ACROSS-MOVEMENT half of `C088` has no observation path. Authoring it without that step would assert lifetime stability while claiming to test GC movement. |
...
| `IRIS-V1-RUNTIME-V083` | `:603` | positive | no-fixture | Re-probed in milestone 2 and RECLASSIFIED. The row supplies a prose schedule requiring a commit to land BEFORE `initialize` returns, which is the same out-of-band scheduling as `V013` and `V025`. It had been tagged executable and was counted as a failure. |
| `IRIS-V1-RUNTIME-V084` | `:604` | negative | executable | Two concrete absent-selector sends each raise `MessageNotFoundError`. |
| `IRIS-V1-RUNTIME-V085` | `:605` | positive | needs-subsystem | Re-probed in milestone 2 and NARROWED to two distinct blockers, neither of which is `migrate_revision` itself. `D-265` makes it an ORDINARY replaceable Method with no privileged path, so it needs no runtime support, and probing confirms both halves of `D-264` already hold: a compatible open commits without the runtime enumerating or invoking anything, so the call log stays empty, and an explicit invocation returns `nil`. What remains is the `ClassRevision` object of `D-266`, which is unimplemented, and its `reactivate()` rejection. Separately the row cannot be transcribed VERBATIM at all, because `D-266` spells the signature `migrate_revision(from: ClassRevision, to: ClassRevision)` while `from` is a reserved keyword in the IRIS-V1-GRAMMAR-C013 inventory, so that parameter name does not parse. That conflict is recorded in `docs/spec-defects-v1.md`. |
| `IRIS-V1-RUNTIME-V086` | `:606` | positive | executable | Re-probed in milestone 2 and now EXECUTABLE. A raw `@x` slot accepts an Integer then a Symbol on one instance while a second instance still reads nil, which is the per-instance `Dynamic<Object>` behaviour the row asserts. |
| `IRIS-V1-RUNTIME-V087` | `:607` | positive | executable | Re-probed in milestone 2 and now EXECUTABLE. An escaped Closure increments its original receiver's `@x` across two calls after the Method returned. The rebind half is not asserted, since `MethodBindingError` needs a rebinding surface that does not exist. |
| `IRIS-V1-RUNTIME-V088` | `:608` | negative | executable | Concrete private-visibility source. Reclassified in milestone 2 once `Dynamic<T>` parsed: three denials already worked, and the Dynamic boundary disables static member checking only, so the runtime denial `D-313` requires still fires. |
...
| `IRIS-V1-RUNTIME-V103` | `:623` | positive | no-fixture | Re-probed in milestone 2 and RECLASSIFIED. The row's `source_text` is a PROSE DESCRIPTION of replacing the `Float64.infinity` getter, not runnable source, and replacing a builtin Class getter is not expressible today. It had been tagged executable and was counted as a failure. |

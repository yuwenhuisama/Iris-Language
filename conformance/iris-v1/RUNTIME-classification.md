...
| `IRIS-V1-RUNTIME-V075` | `:595` | positive | needs-subsystem | Re-probed in milestone 2: the collections reason is stale. It now needs `Object` to supply the default `==` that `C005` requires, so it shares the root-Class blocker recorded in `docs/spec-defects-v1.md`. A Class declaring its own `==` already dispatches correctly. |

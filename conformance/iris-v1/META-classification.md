...
`spec/iris-v1/08-modules-metaprogramming.md` states 51 vector rows. 11 are
committed here; the remaining 40 are NOT yet transcribed and are bucketed below
...
| Reflection views | `V344`, `V416`, `V424`, `V425`, `V432` | `IRIS-V1-META-C100` and `C107` expose `Reflection::Object` ivar APIs and package-scoped reflection whose authorization is ambient to the executing package. The reflection surface these rows read does not exist. `V416` additionally needs `open module P::M` across two files in one package. |
| Cross-package import | `V346`, `V347`, `V348`, `V349`, `V418`, `V438` | Re-probed row by row. All six need ONE subsystem: multi-package fixture loading with per-package export tables, DIRECT-IMPORT activation of static open extensions, and `IRIS-V1-META-C049` replacement authorization at the import site. `D-432`'s three resolution tiers are implemented and observed by `IRIS-V1-CONTROL-V351`, but only for a Module already loaded into this runtime. The facade spellings `export import` and `export from` now PARSE, which was a separate implementation gap. Three findings correct earlier notes: `IRIS-V1-META-C016` requires a consumer to DIRECTLY IMPORT a Module to activate its exported static open extension, and today an `open class` takes effect immediately with no import at all, so there is no merged extension member for `C049` to guard; `D-230` ends with "Same-module open override marking remains separate", so `C049` governs cross-module merging only and `V349` cannot be reduced to a same-package rejection; and `V348`'s observable is IMPORT order picking the winner, which was measured NOT to apply -- reordering two `import` lines leaves the result unchanged while declaration order alone decides it -- so transcribing it against same-package `open class` would assert a substitute mechanism. `V347`'s authorized lane additionally needs an import-site `override` marker that `import_spec ::= ordinary_name import_alias?` does not provide. |

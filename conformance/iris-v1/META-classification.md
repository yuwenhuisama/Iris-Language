...
| Cross-package import | `V346`, `V347`, `V348`, `V349`, `V418`, `V438` | Multi-package LOADING now exists: a fixture tree resolves `dependencies` per `IRIS-V1-META-C006`, orders dependencies before dependents per `C017`, aborts on a missing or cyclic dependency per `C007`, and loads the ordered packages onto ONE runtime so a consumer reaches what its dependency exported. The facade spellings `export import` and `export from` parse, and the v1.24 errata `IRIS-V1-GRAMMAR-C069` supplies the `C049` `override` marker. What remains for every row is `IRIS-V1-META-C016` DIRECT-IMPORT activation: an `open class` today takes effect globally and immediately, so removing a consumer's import does NOT deactivate the extension, and there is no merged extension member for `C049` to guard. `V346` and `V438` additionally observe `IRIS-STATIC-MEMBER-NOT-FOUND`, and static member-existence checking does not exist at all -- an ordinary `A.new().nothing()` is equally undiagnosed, which is the same blocker `IRIS-V1-TYPES-V208` carries. |

...
`spec/iris-v1/08-modules-metaprogramming.md` states 51 vector rows. 6 are
committed here; the remaining 45 are NOT yet transcribed and are bucketed below
...
| Permissions and policy | `V359`, `V362`, `V363`, `V421`, `V422`, `V423`, `V435` | `IRIS-V1-META-C009` makes a permission a manifest REQUEST that only Host configuration can grant, and forbids a package from self-authorizing. The loader models no permission field, and there is no Host grant surface to check one against. |

...
本章给出 Iris v1 的心智模型：每个运行时值都是对象，行为是消息发送，动态行为受静态承诺约束。Iris v1 目前只是已冻结的语言规范。没有编译器、解释器、REPL、playground、包管理器或标准库可用来执行 Iris 程序。请把这里的每个代码块都当成通过语法检查的教学材料，而不是可运行内容。
...
Iris 从根上就是面向对象的。`Counter` 是一个 Class 对象。`Counter.new()` 向这个 Class 对象发送 `new` 消息。`counter.add` 读取一个实例 Method，并产生一个 BoundMethod，它可以存入可调用绑定。具名 `fun` 声明没有一个名为 Function 的独立运行时类别。
